...
Slightly higher median debt-to-income ratio for those who have not fully paid their loan off.

```{r, fig.height = 5, fig.width = 7}
p5 <- loan_data %>%
   ggplot(aes(not.fully.paid, dti)) +
   geom_boxplot(aes(color=not.fully.paid)) +
   theme_minimal() +
   xlab("Loan Payment Status") +
   ggtitle("Debt to Income Ration vs Loan Payment Status") +
   scale_colour_manual(values=c('#009fe3','#25274d'))
plot(p5)
```
...
I need to oversample the minority class (those who haven't paid) - the class split looks alot better now in our training set
...
Once i've trained the model I then use it to predict the values in our test set. Here's a table of predicted vs actual results.

```{r, echo=TRUE, include=TRUE}
table(predicted.svm, loan_test$not.fully.paid, dnn = c("Predicted","Actual"))
```

The model has done well in predicting those that will pay off their loan (0's), but not so well for those that won't (1's). Let's take a look at the ROC and AUC.
...
Here's a table of the predicted vs actual results.
...
Although I looked online and I think the highest AUC I could find was about 0.7 for this dataset

## Lessons learnt?
1. Choose an easy dataset, especially for an interview workshop :)
2. Don't always jump to the fancy models - I spent alot of time trying to tune the SVM model when I should have started with the logistic regression model and might have had time to explore other models.

## Next Steps & Questions?
- Random Forest, GBM, Ensemble models
- Feature engineering
- Use the full dataset (I found a cut-down version online to save me time on the computation)
